...
import org.firstinspires.ftc.teamcode.subsystem.PIDF_Tuning;
@Configurable
...
public class starterbot extends OpMode {
    ...
    private boolean flywheelToggleUp = false;
    ...
    PIDF_Tuning flywheelPIDFController;
    @Override
    public void init(){
        prev1 = new Gamepad();
        curr1 = new Gamepad();
        ...
        flywheelPIDFController = new PIDF_Tuning(Kp, Ki, Kd, Kf);
    }
    ...
    public void loop() {
        ...
        if(curr1.dpad_up && !prev1.dpad_up) flywheelToggle = !flywheelToggle;
        if(curr1.dpad_down && !prev1.dpad_down) flywheelToggleUp = !flywheelToggleUp;
        ...
        if(flywheelToggle) {
            flywheel.setPower(0.735);
            flywheelToggleUp = false;
        } else if(flywheelToggleUp) {
